...
int main(int argc, char* argv[]){
  ...
  for(unsigned i=0; i < stupidNotchStrat->nOperations(); i++){
    if(!TString(stupidNotchStrat->getOperation(i)->description()).Contains("ALFA")){
  ...
  AnalysisFlow analysis(argv[0], run, AnalysisFlow::kWaisPulserAndNonRF, strat, AnitaDataset::kDefault);
  analysis.doAnalysis();
